...
func TestBuildCreatesResourcesInStableExecutionOrder(t *testing.T) {
	cfg := config.Config{
		...
		ASDF: config.ASDF{
			...
			ToolVersions: []config.ASDFToolVersion{
				{Path: "/Users/nix/.tool-versions", Tools: map[string]string{"ruby": "3.3.6"}},
			},
		},
		Casks:       []string{"ghostty"},
		Directories: []string{"/Users/nix/code"},
		Repos: []config.Repo{
			{Path: "/Users/nix/code/kitout", URL: "git@example.com:kitout.git", Branch: "main"},
		},
	...
	want := []string{
		"brew:asdf",
		"asdf_plugin:ruby",
		"asdf_tool_versions:/Users/nix/.tool-versions",
		"cask:ghostty",
