...
- `make smoke-distribution` passes on macOS, covering temporary starter
  configs, `init`, `doctor`, `status`, `apply --dry-run`, a temp-only
  directory copy apply, and a safe login-shell status/dry-run plan
