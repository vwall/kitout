# Documentation Manifest

Generated Kitout starter documentation.

## Files

- `AGENTS.md`
- `CONTRIBUTING.md`
- `README.md`
- `ROADMAP.md`
- `SECURITY.md`
- `docs/architecture/architecture-overview.md`
- `docs/charter/project-charter.md`
- `docs/cli/cli-spec.md`
- `docs/config/config-spec.md`
- `docs/governance/rfcs.md`
- `docs/install/installation-and-distribution.md`
- `docs/product/naming-positioning.md`
- `docs/product/product-brief.md`
- `docs/references/references.md`
- `docs/resources/brew.md`
- `docs/resources/cask.md`
- `docs/resources/directory.md`
- `docs/resources/macos-defaults.md`
- `docs/resources/repo.md`
- `docs/resources/shell.md`
- `docs/resources/symlink.md`
- `docs/runtime/resource-model.md`
- `docs/testing/test-strategy.md`
- `examples/kitout.private.yaml.example`
- `examples/kitout.yaml`
- `rfcs/0001-core-resource-model.md`
- `rfcs/0002-config-format.md`
- `rfcs/0003-command-surface.md`
